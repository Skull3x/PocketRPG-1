  ...
  public function onCommand(Command $cmd, CommandSender $p, $label, array $args) {
    switch($cmd->getName()) {
      case "endquest":
        switch($args[0]) {
          case "1":
              if($p->hasPermission("quest.1.cancomplete") && $p->getInventory()->getId($this->config->get("Item_needed_quest_1"))->getCount() >= $this->config->get("Amount_needed_quest_1")) {
              $p->getInventory()->removeItem($this->config->get("Item_needed_quest_1"));
              ...
              $amount_1 = $this->config->get("Amount_quest_1");
              $gold_1 = Item::get(Item::$this->config->get("Item_quest_1"), 0, $amount_1);
